...
- `src/render/` -- `Renderer` is the Three.js bridge (strictly top-down `OrthographicCamera`); `PrimitiveFactory` is the reskin seam: swap it (same interface) to change the look. `trackPlacement.js` and `launchPlacement.js` are the pure placement helpers (track path; launch flight, entry queue, return glide), tested headless in `tests/render`.
