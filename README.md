...
| `track.launchSpacing` (changed) | 1 | Minimum gap between runners, along the track only: a launch waits for it and a runner never closes in beyond it (0 = pass-through) |
...
| `rules.loseMode` (new) | allSlotsBlocked | allSlotsBlocked: lose as soon as every slot is blocked and nothing moves. deadlock: only if no parked unit could hit a block |
...
| `render.slotCounter.offsetX` (new) | 1.4 | Free-slot counter "N/5": cells right of the last slot |
| `render.slotCounter.offsetY` (new) | 0 | Free-slot counter: cells below the slot row |
| `render.slotCounter.height` (new) | 0.7 | Free-slot counter: text height, cells |

Removed: `inventory.compactReserve` (replaced by the column shift), `rules.detectDeadEndsEarly` (replaced by
